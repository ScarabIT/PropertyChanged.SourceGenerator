...
public class RaisePropertyChangingDefinitionTests : TestsBase
{
    ...
    public void RaisesIfEventButNoRaiseMethodOnBaseClass()
    {
        ...
        this.AssertThat(input, It.HasDiagnostics(
            // (6,22): Warning INPC0028: Could not find any suitable methods to raise the PropertyChanging event defined on a base class
            // Derived
            Diagnostic("INPC0028", @"Derived").WithLocation(6, 22)
        ));
    }
    ...
    public void RaisesIfMethodOnBaseClassIsPrivate()
    {
        ...
        this.AssertThat(input, It.HasDiagnostics(
            // (7,22): Warning INPC029: Found one or more methods called 'OnPropertyChanging' to raise the PropertyChanging event, but they had an unrecognised signatures or were inaccessible. No PropertyChanging events will be raised from this type
            // Derived
            Diagnostic("INPC029", @"Derived").WithLocation(7, 22)
        ));
    }
    ...
    public void RaisesIfNonVirtualBaseMethodAndOverrideRequired()
    {
        ...
        this.AssertThat(input, It.HasFile("Derived", rewriters).HasDiagnostics(
            ...
            // (9, 14): Warning INPC035: [DependsOn("Bar")] specified, but PropertyChanging events will not be raised because the method to raise PropertyChanging events 'OnPropertyChanged' cannot defined or overridden by the source generator
            // DependsOn("Bar")
            Diagnostic("INPC035", @"DependsOn(""Bar"")").WithLocation(9, 14)
        ));
    }
